...
public class LocationValidator {
	...
	public static LocationCoordinates validateAndFetchCoordinates(String cityName, String regionName,
		...
		try {
			...
			conn.setRequestProperty("User-Agent", config.getUserAgent()); // Set the User-Agent header

			int responseCode = conn.getResponseCode();
			if (responseCode == 400) {
				System.out.println("Invalid request (400): Check the parameters.");
				return new LocationCoordinates(0, 0, false); // Indicate invalid city
			}
			...
			if (rootNode.isArray() && rootNode.size() > 0) {
				...
				return new LocationCoordinates(lat, lon, true); // City is valid
			} else {
				System.out.println("Location not found.");
				return new LocationCoordinates(0, 0, false); // City not found
			}

		} catch (Exception e) {
			e.printStackTrace();
			return new LocationCoordinates(0, 0, false); // Return invalid in case of any failure
		}
	}
